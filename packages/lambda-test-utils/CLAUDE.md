...
- `tsconfig.lib.json` / `tsconfig.spec.json` override `target: ES2022` (`module`/`moduleResolution` need no override — `tsconfigs-extend` already sets `ESNext`/`bundler`, which is what the rollup pipeline needs), mirroring `aws-wrappers` for the same reasons: `package.json` declares `engines.node: >=18`, and pinning `ES2022` guards against the inherited `target: "ESNext"` floating to syntax Node 18 can't run, while also avoiding ES5 IIFE class emission.
